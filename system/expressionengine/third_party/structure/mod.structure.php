...
class Structure
{
	...
	function nav_main()
	{
		...
		$css_id = $this->EE->TMPL->fetch_param('css_id', 'nav');

		// Remove the id tag if set to "none"
		if (strtolower($css_id) == "none")
		{
			$css_id = NULL;
		}	
		else
	...
	function nav_sub()
	{
		...
		$limit_depth = $this->EE->TMPL->fetch_param('limit_depth');
		if ($limit_depth == '')
		{
			$depth_limit = FALSE;
		}
	...
	function breadcrumb()
	{
		...
		if ($inc_here)
		{
			...
			if ($here_as_title)
			{
				$crumbs[] = !empty($wrap_here) ? "<{$wrap_here}>{$this->sql->get_entry_title($entry_id)}</{$wrap_here}>" : $this->sql->get_entry_title($entry_id);
			}
			else
			{
	...
	function titletrail()
	{
		...
		$title_array = array();
		$title_array[] = $this->sql->get_entry_title($entry_id);			
				...
				if ($changed !== TRUE)
				{
					...
					else
					{
						if(isset($site_pages['uris']))
						{
							...
							{
								str_replace($prevUri, $uri, $site_pages['uris'][$entry_id]);
							}
