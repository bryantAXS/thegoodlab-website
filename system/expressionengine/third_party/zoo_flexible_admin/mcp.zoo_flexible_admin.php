...
class Zoo_flexible_admin_mcp {
	...
	function index()
	{
		...
		$theme_url = $this->EE->config->item('theme_folder_url') . 'third_party/zoo_flexible_admin';

		$cp_url = $this->EE->config->item('site_url');

		$ACT_script_path = str_replace("?","",$this->EE->functions->fetch_site_index());
		
		$js_url =$urlparts["path"].'?D=cp&amp;C=javascript&amp;M=load&amp;package=zoo_flexible_admin&amp;file=';
		...
		foreach($js as $file){
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.$this->EE->config->item('theme_folder_url').'third_party/zoo_flexible_admin/javascript/'.$file.'.js"></script>');
		}
