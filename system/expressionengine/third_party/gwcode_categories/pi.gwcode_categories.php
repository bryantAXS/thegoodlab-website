...
class Gwcode_categories {
	...
	public function __construct() {
		...
		if($mode == 'single') { // add default tagdata --> all category groups and their categories
			if($this->style != 'linear') {
				$this->gw_tagdata = LD.'group_heading'.RD.'<h2 class="gwcode_categories">'.LD.'cat_group_name'.RD.'</h2>'.LD.'/group_heading'.RD.LD.'cat_name'.RD;
			}
			else {
				$this->gw_tagdata .= LD.'if group_start'.RD.'<h2 class="gwcode_categories">'.LD.'cat_group_name'.RD.'</h2>'.LD.'/if'.RD.LD.'if group_end'.RD.LD.'cat_name'.RD.LD.'if:else'.RD.LD.'cat_name'.RD.', '.LD.'/if'.RD;
			}
		}
	...
	private function _generate_output() {
		...
		for($gw_i=0;$gw_i<$output_limit;$gw_i++) {
			...
			$var_values_arr = array(
								'site_id' => $this->categories[$gw_i]['site_id'],
								'cat_count' => $gw_i+1,
								'cat_id' => $this->categories[$gw_i]['cat_id'],
								'cat_name' => $this->categories[$gw_i]['cat_name'],
								'cat_url_title' => $this->categories[$gw_i]['cat_url_title'],
								'cat_description' => $this->categories[$gw_i]['cat_description'],
								'cat_image' => $this->categories[$gw_i]['cat_image'],
								'cat_order' => $this->categories[$gw_i]['cat_order'],
								'parent_id' => $this->categories[$gw_i]['parent_id'],
								'cat_group_id' => $this->categories[$gw_i]['cat_group_id'],
								'cat_group_name' => $this->categories[$gw_i]['cat_group_name'],
								'depth' => $this->categories[$gw_i]['depth'],
								'results' => $output_limit,
								'results_total' => $results_total,
								'group_start' => $group_start,
								'has_children' => $this->categories[$gw_i]['has_children'],
								'entry_count' => $this->categories[$gw_i]['entry_count'],
								'li_depth' => $current_ul_depth,
								'complete_path' => implode('/',$this->categories[$gw_i]['complete_path']),
								'group_end' => $last_cat_in_group,
								'cat_count_in_group' => $cat_count_in_group,
								'output_path' => implode('/',$output_path_arr),
								'has_children_in_output' => $has_children_in_output 
								);
			...
			for($i=1;$i<=$this->max_depth_in_output;$i++) {
				...
				$var_values_arr['depth'.$i.'_start'] = ${'depth'.$i.'_start'};
				$var_values_arr['depth'.$i.'_end'] = ${'depth'.$i.'_end'};
				$var_values_arr['depth'.$i.'_start_count'] = ${'depth'.$i.'_start_count'};
				$var_values_arr['depth'.$i.'_end_count'] = ${'depth'.$i.'_end_count'};
			}
			...
			if($this->custom_fields == 'yes' && !empty($this->custom_fields_arr)) {
				foreach($this->custom_fields_arr as $cf_group_id => $cf_group_arr) {
					foreach($this->custom_fields_arr[$cf_group_id] as $cf_arr_key => $cf_field_array) {
						...
						$var_values_arr[$cf_field_array['field_name']] = ($this->categories[$gw_i]['cat_group_id'] == $cf_group_id) ? $this->categories[$gw_i][$cf_field_array['field_name']] : '';
					}
				}
			}
			...
			if($this->style != 'linear') {
				...
				if($last_cat_in_group) {
					if($group_heading_count > 0) { // {group_heading}..{/group_heading} tag pair was found
						...
						$group_heading = str_replace(array('{cat_group_id}','{cat_group_name}','{cat_count_in_group}'),array($this->categories[$gw_i]['cat_group_id'],$this->categories[$gw_i]['cat_group_name'],$cat_count_in_group),$matches_arr[1][0]);
	...
	private function _get_categories($sql) {
		...
		// the $cat_array now contains all categories we need. It's not in the correct order yet, so let's create the correct order:
