...
$( function( base ) {
	...
	base.getLevel = function( path )
	{
		...
		$.post(
			...
			{
				base.showErrorResponse( response.responseText );
			}
		);
	};
	...
	base.setup = function()
	{
		...
		$( document ).delegate( '.ce_cache_delete', 'click', function( e )
		{
			...
			var is_local = item.data('path').substring(0,5) == 'local';
			...
			if ( type == 'folder' ) //folder
			{
				...
				dialogHTML += '<p>' + base.lang.delete_child_items_confirmation.replace( '%s', $( this ).closest('li').find('.ce_cache_name').text() ) + '</p>' +
					'<p';
				if ( ! is_local ) //hide the refresh option if this is a global item
				{
