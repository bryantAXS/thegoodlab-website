...
class Ce_cache_cp
{
	...
	public function items_to_html_list( $items )
	{
		...
		foreach ( $items as $item )
		{
			...
			if ( $item['type'] == 'folder' )
			{
				//' . BASE . AMP . 'C=addons_modules' . AMP . 'M=show_module_cp' . AMP . "module=ce_cache" . AMP . 'method=delete_children' . AMP . 'driver=' . $driver . AMP . 'path=' . urlencode( $item['id_full'] ) . '
			...
			{
				//' . BASE . AMP . 'C=addons_modules' . AMP . 'M=show_module_cp' . AMP . "module=ce_cache" . AMP . 'method=delete_item' . AMP . 'driver=' . $driver . AMP . 'item=' . urlencode( $item['id_full'] ) . '
				//' . BASE . AMP . 'C=addons_modules' . AMP . 'M=show_module_cp' . AMP . "module=ce_cache" . AMP . 'method=view_item' . AMP . 'driver=' . $driver . AMP . 'item=' . urlencode( $item['id_full'] ) . '
