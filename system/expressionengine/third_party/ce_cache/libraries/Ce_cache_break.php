...
class Ce_cache_break
{
	...
	public function break_cache( $items = array(), $tags = array(), $refresh = TRUE, $refresh_time = 1 )
	{
		...
		foreach ( $items as $item )
		{
			...
			if ( $sub  == 'local' ) //a local item or path
			{
		...
		$can_recreate = ( $this->async && ( function_exists( 'curl_init' ) || function_exists( 'fsockopen' ) ) );
		...
		if ( $can_recreate ) //delete and refresh the items
		{
			...
			foreach ( $refreshers as $item => $time )
			{
				...
				//trim the prefix and 'local/' from the beginning of the path
				$path = substr( $item, strlen( $prefix ) + 6 );
	...
	public function break_cache_hook( $ids, $secret )
	{
		...
		$results = $this->EE->db->query( "SELECT ct.entry_id, ct.channel_id, ct.title, ct.url_title, ct.entry_date, ct.edit_date, c.channel_title, c.channel_name
	...
	/**
	 * A simple method that leverages cURL to make a quick GET request. If the request is asynchronous, it has a quick timeout (200ms or 1 second, depending on the PHP version) that essentially calls the URL without waiting around for a response.
	 *
	 * @param string $url
	 * @return bool
	 */
	...
	public function fsockopen_it( $url )
	{
		if ( function_exists( 'fsockopen' ) ) //no cURL, try fsocketopen
		{
			...
			$out = 'GET ' . $parts['path'] . ( ! empty( $parts['query'] ) ? '?' . $query : '' ) . ' HTTP/1.1' . "\r\n";
			$out .= 'Host: ' . $parts['host'] . "\r\n";
