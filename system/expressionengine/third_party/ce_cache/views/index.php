<?php
	...
	foreach( $drivers as $driver )
	{
		$this->table->add_row(
			...
			( $driver != 'memcached' && $driver != 'dummy' ) ? '<a href="' . BASE . AMP . 'C=addons_modules' . AMP . 'M=show_module_cp' . AMP . "module={$module}" . AMP . 'method=view_items' . AMP . "driver={$driver}" . '">' . lang( "{$module}_view_items" ) . '</a>' : '&ndash;',
			( $driver != 'memcached' && $driver != 'dummy' ) ? '<a href="' . BASE . AMP . 'C=addons_modules' . AMP . 'M=show_module_cp' . AMP . "module={$module}" . AMP . 'method=clear_cache' . AMP . "driver={$driver}" . AMP . 'site_only=y">' . lang( "{$module}_clear_cache_site" ) . '</a>' : '&ndash;',
