...
class Ce_cache_ext
{
	...
	{
		if ( strpos( $row['template_data'], '{exp:ce_cache:escape}' ) !== FALSE )
		{
			preg_match_all( '@\{(exp\:ce\_cache\:escape\:(\S+)).*\}(.*)\{/\\1\}@Uis', $row['template_data'], $matches, PREG_SET_ORDER);
	...
	public function multi_updated_shut_down()
	{
		...
		//break the entries
		$this->break_cache( array_unique( $entry_ids ) );
	}
	 ...
	 * @return void
	 */
	private function break_cache( $entry_ids )
	{
